...
@st.cache_data(ttl=3600)  # 快取1小時，避免頻繁呼叫yfinance API
def get_stock_info(symbol, is_taiwan_stock=True):
    """獲取股票資訊，包括類型、股息和分割資料"""
    try:
        # 清理股票代號
        symbol = str(symbol).strip()
        
        # 台股需要加上.TW後綴，但要檢查是否已經有了
        if is_taiwan_stock:
        ...
        print(f"正在查詢股票: {ticker_symbol}")  # Debug用
        ...
        try:
            ...
            # 如果無法獲取info，可能是股票代號錯誤，嘗試其他格式
            if not info or 'regularMarketPrice' not in info:
                if is_taiwan_stock and ticker_symbol.endswith('.TW'):
                    # 嘗試 .TWO 後綴
                    ticker_symbol_alt = symbol + '.TWO'
                    stock_alt = yf.Ticker(ticker_symbol_alt)
                    info_alt = stock_alt.info
                    if info_alt and 'regularMarketPrice' in info_alt:
                        stock = stock_alt
                        info = info_alt
                        ticker_symbol = ticker_symbol_alt
                        print(f"改用 {ticker_symbol} 成功")
        except Exception as e:
            print(f"獲取股票資訊失敗: {e}")
            info = {}
            quote_type = 'Unknown'
        ...
        # 獲取股票分割資料
        split_info = get_split_info(stock)
        
        result = {
            'quote_type': quote_type,
            'ticker_used': ticker_symbol,  # 記錄實際使用的ticker
            **dividend_info,
            **split_info
        }
        
        print(f"股票 {symbol} 資訊獲取成功: {result}")
        return result
    
    except Exception as e:
        print(f"獲取 {symbol} 股票資訊時發生錯誤: {str(e)}")
        return {
            'quote_type': 'Error',
            'ticker_used': symbol,
...
def get_dividend_info(stock):
    ...
    try:
        ...
        if not dividends.empty:
            ...
            if not recent_dividends.empty:
                last_dividend_date = recent_dividends.index[-1].strftime('%Y-%m-%d')
                last_dividend_amount = float(recent_dividends.iloc[-1])
                total_dividends_12m = float(recent_dividends.sum())
            else:
                # 如果沒有近12個月的，取最近一次
                last_dividend_date = dividends.index[-1].strftime('%Y-%m-%d')
                last_dividend_amount = float(dividends.iloc[-1])
                total_dividends_12m = last_dividend_amount
        else:
            last_dividend_date = 'N/A'
            last_dividend_amount = 0
            total_dividends_12m = 0
    ...
    return {
        ...
        'total_dividends_12m': total_dividends_12m
    }

def get_split_info(stock):
    """獲取股票分割資訊"""
    try:
        splits = stock.splits
        if not splits.empty:
            ...
            if not recent_splits.empty:
                last_split_date = recent_splits.index[-1].strftime('%Y-%m-%d')
                split_ratio = recent_splits.iloc[-1]
                last_split_ratio = f"1:{split_ratio:.2f}"
            else:
                # 如果沒有近5年的，取最近一次
...
def enhance_holdings_with_stock_info(holdings_df, person):
    """為持股資料增加股票資訊"""
    ...
    # 準備新欄位
    enhanced_df['股票類型'] = ''
    enhanced_df['使用代號'] = ''  # 顯示實際使用的ticker符號
    enhanced_df['最近股息日期'] = ''
    enhanced_df['最近股息金額'] = 0.0
    enhanced_df['12月股息總額'] = 0.0
    enhanced_df['最近分割日期'] = ''
    enhanced_df['分割比例'] = ''
    ...
    if person == 'os':
        # 海外股票 - 尋找可能的代號欄位
        symbol_col = None
        possible_symbol_cols = ['股票代號', 'Symbol', 'Ticker', 'Code', '代號']
        
        for col_name in possible_symbol_cols:
            if col_name in enhanced_df.columns:
                symbol_col = col_name
                break
        
        # 如果都找不到，使用第一個欄位
        if not symbol_col and len(enhanced_df.columns) > 0:
            symbol_col = enhanced_df.columns[0]
            st.warning(f"海外股票找不到代號欄位，使用 '{symbol_col}' 作為股票代號")
        
        if symbol_col:
            total_stocks = len(enhanced_df)
            for idx, (index, row) in enumerate(enhanced_df.iterrows()):
                symbol = row[symbol_col]
                if pd.notna(symbol) and str(symbol).strip():
                    status_text.text(f"正在查詢 {symbol} ({idx+1}/{total_stocks})...")
                    progress_bar.progress((idx + 1) / total_stocks)
                    
                    stock_info = get_stock_info(str(symbol).strip(), is_taiwan_stock=False)
                    enhanced_df.at[index, '股票類型'] = stock_info['quote_type']
                    enhanced_df.at[index, '使用代號'] = stock_info['ticker_used']
                    enhanced_df.at[index, '最近股息日期'] = stock_info['last_dividend_date']
                    enhanced_df.at[index, '最近股息金額'] = stock_info['last_dividend_amount']
                    enhanced_df.at[index, '12月股息總額'] = stock_info['total_dividends_12m']
                    enhanced_df.at[index, '最近分割日期'] = stock_info['last_split_date']
                    enhanced_df.at[index, '分割比例'] = stock_info['last_split_ratio']
                else:
                    enhanced_df.at[index, '股票類型'] = 'N/A'
                    enhanced_df.at[index, '使用代號'] = 'N/A'
    else:
        # 台股
        if '股票代號' in enhanced_df.columns:
            total_stocks = len(enhanced_df)
            for idx, (index, row) in enumerate(enhanced_df.iterrows()):
                symbol = row['股票代號']
                if pd.notna(symbol) and str(symbol).strip():
                    ...
                    stock_info = get_stock_info(str(symbol).strip(), is_taiwan_stock=True)
                    enhanced_df.at[index, '股票類型'] = stock_info['quote_type']
                    enhanced_df.at[index, '使用代號'] = stock_info['ticker_used']
                    enhanced_df.at[index, '最近股息日期'] = stock_info['last_dividend_date']
                    enhanced_df.at[index, '最近股息金額'] = stock_info['last_dividend_amount']
...
def render_holdings_table(person, holdings_df):
    ...
    # 增強持股資料
    with st.spinner("正在獲取股票額外資訊..."):
        enhanced_df = enhance_holdings_with_stock_info(holdings_df, person)
    ...
    if person == 'os':
        ...
        # 格式化股息金額
        for col in ['最近股息金額', '12月股息總額']:
            if col in display_df.columns:
                display_df[col] = display_df[col].apply(lambda x: f"${x:.4f}" if x > 0 else 'N/A')
        ...
        for col in ['最近股息金額', '12月股息總額']:
            if col in display_df.columns:
                display_df[col] = display_df[col].apply(lambda x: f"NT${x:.2f}" if x > 0 else 'N/A')
        ...
        # 重新排序欄位 - 加入新的股票資訊欄位
        column_order = [
            '持股標籤', '股票類型', '使用代號', '總持有股數', '目前股價', 
            '總投入成本', '目前總市值', '未實現損益', '報酬率',
            '最近股息日期', '最近股息金額', '12月股息總額',
            '最近分割日期', '分割比例'
        ]
    ...
    # 顯示股票類型統計
    if '股票類型' in enhanced_df.columns:
        st.markdown("### 📊 投資組合類型分析")
        type_counts = enhanced_df['股票類型'].value_counts()
        
        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**股票類型分布:**")
            for stock_type, count in type_counts.items():
                st.write(f"• {stock_type}: {count} 檔")
        
        with col2:
            # 計算近期有股息的股票數量
            dividend_stocks = enhanced_df[enhanced_df['最近股息日期'] != 'N/A'].shape[0]
            split_stocks = enhanced_df[enhanced_df['最近分割日期'] != 'N/A'].shape[0]
            
            st.markdown("**股息與分割統計:**")
            st.write(f"• 近12個月有配息: {dividend_stocks} 檔")
            st.write(f"• 近5年有股票分割: {split_stocks} 檔")
...
def main():
    ...
    # 分頁標籤
    tab1, tab2, tab3, tab4 = st.tabs(["🚘 Jason定期定額", "👩🏻 Rita定期定額", "👨🏻 Ed定期定額", "🇬🇧 富邦英股總覽"])
