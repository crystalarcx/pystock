...
def render_user_selection():
    ...
    # 使用者選項配置 - 移除ed_combined和os，新增ed_overseas
    user_options = [
...
def render_ed_overseas_summary(schwab_total_usd, cathay_total_usd, fubon_total_usd, fubon_total_ntd):
    ...
    with col3:
        st.markdown(f"""
        <div class="fubon-card">
            <div style="font-size: 1rem; font-weight: 600; margin-bottom: 1rem; opacity: 0.9;">🇬🇧 富邦英股</div>
            <div style="font-size: 2.5rem; font-weight: 700; margin-bottom: 0.5rem;">{format_currency(fubon_total_usd, 'USD')}</div>
            <div style="opacity: 0.8;">英股總市值</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        <div class="metric-card" style="border: none; background: #e8f5e9;">
            <div style="font-size: 1rem; font-weight: 600; margin-bottom: 1rem; color: #388e3c; opacity: 0.9;">總資產 (USD)</div>
            <div style="font-size: 2.5rem; font-weight: 700; color: #1b5e20; margin-bottom: 0.5rem;">{format_currency(total_combined_usd, 'USD')}</div>
            <div style="opacity: 0.8;">三平台合計</div>
        </div>
        """, unsafe_allow_html=True)
