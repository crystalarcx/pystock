    ...
    if person == 'os':
        ...
        with col2:
            ...
            <div class="metric-card">
                ...
                <div class="metric-change">{format_currency(total_value_ntd, 'TWD')}</div>
            </div>
            """, unsafe_allow_html=True)
        ...
        with col4:
            ...
            <div class="metric-card">
                ...
                <div class="metric-value {fx_class}">{format_currency(fx_pl_ntd, 'TWD')}</div>
        ...
        with col5:
            ...
            <div class="metric-card">
                ...
                <div class="metric-value {total_class}">{format_currency(total_pl_ntd, 'TWD')}</div>
                <div class="metric-change">{format_percentage(return_rate_ntd)}</div>
            </div>
            """, unsafe_allow_html=True)
                ...
                if all(col in dca_df.columns for col in required_dca_columns):
                    ...
                    <div class="dca-card">
                        ...
                        <div style="font-size: 0.9rem; margin-top: 10px;">
                            {"".join(dca_items) if dca_items else '<div style="text-align: center; opacity: 0.8;">暫無設定資料</div>'}
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
                else:
                    ...
                    <div class="dca-card">
                        <div class="metric-label">📅 定期定額設定</div>
                        <div style="text-align: center; opacity: 0.8; margin-top: 10px;">資料格式錯誤</div>
                    </div>
                    """, unsafe_allow_html=True)
            else:
                st.markdown(f"""
...
def render_holdings_table(person, holdings_df):
    ...
    if person == 'os':
        ...
        # 格式化數字欄位
        format_columns = {
            '目前美價': lambda x: format_currency(x, 'USD'),
            '總投入成本(USD)': lambda x: format_currency(x, 'USD'),
            '目前總市值(USD)': lambda x: format_currency(x, 'USD'),
            '目前總市值(NTD)': lambda x: format_currency(x, 'TWD'),
            '未實現損益(USD)': lambda x: format_currency(x, 'USD'),
            '未實現報酬率': lambda x: format_percentage(x),
            '投資損益(不計匯差,NTD)': lambda x: format_currency(x, 'TWD'),
            '匯率損益(NTD)': lambda x: format_currency(x, 'TWD'),
            '總未實現損益(計算匯差,NTD)': lambda x: format_currency(x, 'TWD'),
            '總未實現損益%': lambda x: format_percentage(x)
        }
