...
# 移除自動更新的 @st.cache_data(ttl=300)，改為手動更新
@st.cache_data
...
def render_charts(person, holdings_df):
    ...
    if person == 'os':
        ...
        try:
            ...
            fig_portfolio.update_layout(height=350, showlegend=True, font_size=10)
...
def main():
    ...
    # 頁腳 - 手機優化
    st.markdown("""
    <div style="text-align: center; padding: 1.5rem; margin-top: 2rem; border-top: 1px solid #e0e0e0; color: #7f8c8d;">
        <p style="font-size: 0.85rem; line-height: 1.4;">💡 手動更新數據 | 📱 手機優化介面 | 📊 投資有風險，請謹慎決策</p>
    </div>
    """, unsafe_allow_html=True)
