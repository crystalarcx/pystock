...
def load_sheet_data(person, data_type, specific_sheet=None):
    """從Google Sheets載入數據"""
    ...
    try:
        ...
        if person == 'ed_overseas':
            # Ed海外總覽特殊處理
            if specific_sheet:
                sheet_config = config[specific_sheet]
...
def get_fubon_uk_summary(fubon_uk_df):
    """從富邦英股數據計算摘要"""
    try:
        if fubon_uk_df.empty:
            return {'total_cost': 0, 'total_value': 0, 'total_pl': 0, 'return_rate': 0, 'holdings_count': 0}
        ...
        for col in fubon_uk_df.columns:
            if '成本' in col:
                cost_col = col
            elif '市值' in col:
                value_col = col
        ...
        total_cost = fubon_uk_df[cost_col].sum() if cost_col else 0
        total_value = fubon_uk_df[value_col].sum() if value_col else 0
        total_pl = fubon_uk_df[pl_col].sum() if pl_col else 0
        return_rate = (total_pl / total_cost) * 100 if total_cost > 0 else 0
        holdings_count = len(fubon_uk_df)
...
def render_ed_overseas_summary(schwab_total_usd, cathay_total_usd, fubon_uk_summary):
    """渲染Ed海外投資摘要卡片"""
    
    total_combined = schwab_total_usd + cathay_total_usd + fubon_uk_summary['total_value']
    ...
    with col3:
        profit_class = 'profit' if fubon_uk_summary['total_pl'] >= 0 else 'loss'
        st.markdown(f"""
        <div class="fubon-uk-card">
            <div style="font-size: 1rem; font-weight: 600; margin-bottom: 1rem; opacity: 0.9;">🇬🇧 富邦英股</div>
            <div style="font-size: 2.5rem; font-weight: 700; margin-bottom: 0.5rem;">{format_currency(fubon_uk_summary['total_value'])}</div>
            <div style="opacity: 0.8; font-size: 0.9rem;">{format_percentage(fubon_uk_summary['return_rate'])} | {fubon_uk_summary['holdings_count']}檔</div>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col4:
        ...
        <div class="metric-card" style="border: none; background: #e8f5e9;">
            ...
            <div style="opacity: 0.8;">海外總投資</div>
        </div>
        """, unsafe_allow_html=True)
...
def main():
    ...
    if person == 'ed_overseas':
        ...
        fubon_uk_df = load_sheet_data('ed_overseas', None, 'fubon_uk')

        # 處理並計算總值
        schwab_total_usd = get_schwab_total_value(schwab_df)
        cathay_total_usd = get_cathay_total_value(cathay_df)
        fubon_uk_summary = get_fubon_uk_summary(fubon_uk_df)
        
        # 渲染摘要
        render_ed_overseas_summary(schwab_total_usd, cathay_total_usd, fubon_uk_summary)
        
        # 使用標籤頁組織內容
        tab1, tab2, tab3 = st.tabs(["📊 各券商詳情", "📈 富邦英股明細", "📋 數據概覽"])
        ...
        with tab2:
            ...
            if not fubon_uk_df.empty:
                render_portfolio_chart(fubon_uk_df, 'os')
                render_holdings_table(fubon_uk_df, 'os')
            else:
                st.info("無富邦英股數據")
        ...
        with tab3:
            ...
            with col1:
                st.metric(
                    label="🇺🇸 嘉信證券總市值",
                    value=format_currency(schwab_total_usd, 'USD'),
                    help="從嘉信證券工作表B欄最下方取得"
                )
            
            with col2:
                st.metric(
                    label="🇹🇼 國泰證券總市值", 
                    value=format_currency(cathay_total_usd, 'USD'),
                    help="從國泰證券工作表F欄計算總和"
                )
            ...
            with col3:
                st.metric(
                    label="🇬🇧 富邦英股總市值",
                    value=format_currency(fubon_uk_summary['total_value']),
                    delta=format_percentage(fubon_uk_summary['return_rate']),
                    help=f"持股檔數: {fubon_uk_summary['holdings_count']}"
                )
