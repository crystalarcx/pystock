...
def load_sheet_data(person, data_type, broker=None):
    ...
    try:
        ...
        # --- 新增讀取 allocation_config 的邏輯 ---
        elif person == 'allocation_config':
            config = SHEET_CONFIGS['allocation_config']
            sheet_id = config['id']
            range_name = config['range']
        else:
...
def get_usd_to_twd_rate():
    ...
    try:
        ...
        if rate:
            st.session_state['usd_twd_rate'] = rate
            return rate
        st.error("從API回應中找不到TWD匯率。")
        return 32.0 # 返回一個預設值
    except Exception as e:
        st.warning(f"無法自動獲取即時匯率，將使用預設值 32.0。錯誤: {e}")
        return 32.0 # 如果API失敗，返回一個合理的預設值
...
def render_user_selection():
    ...
    user_options = [
        ...
        {'key': 'allocation', 'icon': '🎯', 'label': '總覽', 'desc': '資產配置'} # 新增按鈕
    ]
...
# (其餘 render_* 函式與您提供的版本相同，此處省略以節省空間)
# --- 原有的 render_* 函式 ---
...
def render_asset_allocation_page():
    ...
    st.info(f"目前的 USD/TWD 匯率約為: **{usd_twd_rate:.4f}** (每30分鐘自動更新)", icon="💹")
    ...
    # 6. 顯示摘要卡片和主表格
    st.markdown(f'<div class="metric-card"><div class="metric-label">總資產 (TWD)</div><div class="metric-value">{format_currency(total_assets_twd)}</div><div style="opacity: 0.8;">Rita台股 + Ed台股 + Ed海外</div></div>', unsafe_allow_html=True)
    
    st.subheader("配置總覽與差距分析")
    st.dataframe(
        final_df.style.format({
            ...
            cmap='RdYlGn', subset=['差距'], vmin=-25, vmax=25
        ),
        use_container_width=True
    )
    ...
    with col1:
        with st.container(border=True):
            ...
            fig_pie.update_layout(legend_title_text='資產種類')
            st.plotly_chart(fig_pie, use_container_width=True)

    with col2:
        with st.container(border=True):
            st.markdown("##### 目前 vs. 理想配置比較")
            fig_bar = go.Figure(data=[
                go.Bar(name='目前配置', x=final_df['種類'], y=final_df['目前配置'], marker_color='#636EFA'),
                go.Bar(name='理想配置', x=final_df['種類'], y=final_df['理想配置'], marker_color='#FECB52')
            ])
