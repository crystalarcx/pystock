...
<style>
    ...
    * {
    ...
    .trade-button.new-holding:hover {
...
def get_google_sheets_service():
    ...
    try:
        ...
        st.error(f"Google Sheets API 設定失敗: {e}")
        return None
...
def get_stock_info(symbol):
    ...
    try:
        ...
        short_name = (
            ...
            symbol  # 如果都找不到，使用原始代號
        )
        
        return short_name
    except Exception as e:
        st.warning(f"無法獲取股票 {symbol} 的資訊: {e}")
        return symbol  # 返回原始代號
...
def parse_number(value):
    """解析數字，處理各種格式 - 加強錯誤處理"""
    if pd.isna(value) or value is None:  # 加入 pd.isna 檢查
        return 0.0
...
def load_sheet_data(person, data_type, broker=None):
    ...
    try:
        ...
        if not values or len(values) < 2:  # 檢查是否至少有標題和一筆數據
            return pd.DataFrame()
        ...
        for row in values:
            ...
            while len(row) < max_cols:
                row.append('')
            normalized_values.append(row[:max_cols])  # 截斷多餘的列
...
def get_asset_allocation_data():
    ...
    try:
        ...
        # 4. Ed 海外投資 - 國泰證券 (使用類別欄位進行分類，國泰的類別在H欄，即第8欄)
...
def render_trading_form(person):
    """渲染交易記錄表單"""
    st.markdown('<div class="trading-form">', unsafe_allow_html=True)
    st.subheader("📝 新增交易記錄")
    ...
    with st.form("trading_form", clear_on_submit=True):
        ...
        with col2:
            stock_symbol = st.text_input("股票代號", placeholder="例如: 2330")
        
        with col3:
            buy_price = st.number_input("買入股價", min_value=0.01, format="%.2f")
        
        with col4:
            buy_shares = st.number_input("買入股數", min_value=1, format="%d")
        
        # 顯示計算結果
        if buy_price > 0 and buy_shares > 0:
            total_cost = buy_price * buy_shares
            st.info(f"總成本: NT${total_cost:,.0f}")
        
        col_btn1, col_btn2, col_spacer = st.columns([1, 1, 2])
        
        with col_btn1:
            new_holding_submitted = st.form_submit_button(
                "新持有", 
                use_container_width=True,
                type="primary"
            )
        ...
        with col_btn2:
            existing_holding_submitted = st.form_submit_button(
                "原本持有", 
                use_container_width=True
            )
    
    # 處理表單提交
    if new_holding_submitted or existing_holding_submitted:
        if not stock_symbol or buy_price <= 0 or buy_shares <= 0:
            st.error("請填寫所有必要欄位！")
            return
        
        # 執行交易記錄寫入
        success_transaction = write_transaction_record(person, trade_date, stock_symbol, buy_price, buy_shares)
        
        if success_transaction:
            if new_holding_submitted:
                # 新持有：額外寫入總覽與損益工作表
                success_holding = write_new_holding_record(person, stock_symbol)
                
                if success_holding:
                    st.success(f"✅ 新持有記錄已成功新增！股票代號: {stock_symbol}")
                else:
                    st.warning("⚠️ 交易記錄已新增，但新增至總覽與損益工作表失敗")
            else:
                st.success(f"✅ 交易記錄已成功新增！股票代號: {stock_symbol}")
            ...
            st.error("❌ 交易記錄新增失敗，請檢查網路連線或稍後再試")
    
    st.markdown('</div>', unsafe_allow_html=True)

def write_transaction_record(person, trade_date, stock_symbol, buy_price, buy_shares):
    """寫入交易記錄到「交易紀錄」工作表"""
    try:
        ...
        # 準備寫入的資料 (A: 日期, B: 代號, C: 股價, F: 總成本, G: 股數)
        # 注意：Column D 和 E 保持空白，所以我們需要在正確位置插入空字串
        values_to_append = [[
            date_str,        # Column A: 交易日期
            stock_symbol,    # Column B: 股票代號  
            buy_price,       # Column C: 買入股價
            '',              # Column D: 空白
            '',              # Column E: 空白
            total_cost,      # Column F: 總成本
            buy_shares       # Column G: 買入股數
        ]]
        
        return append_to_sheet(sheet_id, worksheet_name, values_to_append)
        
    except Exception as e:
        st.error(f"寫入交易記錄失敗: {e}")
        return False

def write_new_holding_record(person, stock_symbol):
    """寫入新持有記錄到「總覽與損益」工作表"""
    try:
        sheet_id = SHEET_CONFIGS[person]['id']
        worksheet_name = '總覽與損益'
        
        # 使用 yfinance 取得股票名稱
        stock_name = get_stock_info(stock_symbol)
        
        # 準備寫入的資料
        values_to_append = [[
            stock_symbol,    # Column A: 股票代號
...
def render_summary_cards(person, holdings_df, dca_df=None):
    ...
    try:
        ...
        st.write("除錯資訊 - 持股數據預覽:", holdings_df.head() if not holdings_df.empty else "數據為空")
...
def render_trend_chart(trend_df):
    ...
    try:
        ...
        if missing_columns:
            st.error(f"趨勢圖缺少必要欄位: {', '.join(missing_columns)}")
            return
        ...
        st.error(f"資產趨勢圖繪製失敗: {str(e)}")
        st.write("除錯資訊 - 趨勢數據預覽:", trend_df.head() if not trend_df.empty else "數據為空")
