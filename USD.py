...
def render_cathay_table(cathay_df):
    ...
    try:
        ...
        # 格式化數值欄位 - 改為美金格式，移除NT前綴
        for col_idx, col in enumerate(display_df.columns):
            try:
                # 檢查是否為數值欄位
                if col_idx >= 1:  # 跳過第一欄(通常是名稱)
                    # 嘗試轉換為數字並格式化為美金
                    display_df[col] = display_df[col].apply(
                        lambda x: format_currency(parse_number(x), 'USD') 
                        if pd.notna(x) and str(x).strip() != '' and parse_number(x) != 0 
                        else x
                    )
            except:
                pass
...
if __name__ == "__main__":
    main()
                 in val and not '$-' in val and not val.startswith('$0'):
                return 'color: #27ae60; font-weight: bold'
            return ''
