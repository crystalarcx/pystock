...
def load_sheet_data(person, data_type):
    ...
    try:
        ...
        if person == 'os' and data_type == 'holdings':
            ...
            # 先檢查實際的column names
            st.write(f"OS Holdings columns: {df.columns.tolist()}")  # Debug用，正式版可移除
        ...
        with col4:
            ...
            if dca_df is not None and not dca_df.empty:
                ...
                if all(col in dca_df.columns for col in required_dca_columns):
                    ...
                    # 使用st.components.v1.html來正確渲染HTML
                    ...
                    <div class="dca-card">
                        ...
                        <div style="font-size: 0.9rem; margin-top: 10px;">
                            {dca_items_html if dca_items_html else '<div style="text-align: center; opacity: 0.8;">暫無設定資料</div>'}
                        </div>
                    </div>
                    """
                    
                    # 使用HTML component來正確渲染
                    components.html(dca_html, height=200)
                else:
                    st.markdown(f"""
                    <div class="dca-card">
                        <div class="metric-label">📅 定期定額設定</div>
