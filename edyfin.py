...
def get_stock_info(symbol, is_taiwan_stock=True):
    ...
    try:
        # 台股需要加上.TW後綴
        ticker_symbol = f"{symbol}.TW" if is_taiwan_stock else symbol
        ...
        try:
            ...
            if not dividends.empty:
                ...
                if not recent_dividends.empty:
                    ...
                    last_dividend_amount = recent_dividends.iloc[-1]
                    total_dividends_12m = recent_dividends.sum()
                else:
        ...
        try:
            ...
            if not splits.empty:
                ...
                if not recent_splits.empty:
                    ...
                    last_split_ratio = f"1:{recent_splits.iloc[-1]:.1f}"
                else:
                    last_split_date = 'N/A'
        ...
        st.warning(f"無法獲取 {symbol} 的股票資訊: {e}")
        return {
            'quote_type': 'Unknown',
            'last_dividend_date': 'N/A',
            'last_dividend_amount': 0,
...
def enhance_holdings_with_stock_info(holdings_df, person):
    ...
    if person == 'os':
        ...
        for col in enhanced_df.columns:
            if '代號' in col or 'Symbol' in col or 'Ticker' in col:
                symbol_col = col
                break
        
        if symbol_col and symbol_col in enhanced_df.columns:
            for idx, row in enhanced_df.iterrows():
                symbol = row[symbol_col]
                if pd.notna(symbol):
                    stock_info = get_stock_info(str(symbol).strip(), is_taiwan_stock=False)
                    enhanced_df.at[idx, '股票類型'] = stock_info['quote_type']
                    enhanced_df.at[idx, '最近股息日期'] = stock_info['last_dividend_date']
                    enhanced_df.at[idx, '最近股息金額'] = stock_info['last_dividend_amount']
                    enhanced_df.at[idx, '12月股息總額'] = stock_info['total_dividends_12m']
                    enhanced_df.at[idx, '最近分割日期'] = stock_info['last_split_date']
                    enhanced_df.at[idx, '分割比例'] = stock_info['last_split_ratio']
    else:
        # 台股
